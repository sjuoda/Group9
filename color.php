...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="colors_dynamic.php">
...
<body>
    ...
    <main>
        <form id="coorForm" action="color.php" method="POST">
            ...
            <label for="colors"> Number of Colors (1-<?php echo $totalColors; ?>)</label>
            <p>
                <input type="number" name="colors" id="colors">
                <span id="colorError" style="display: none;">Error: Colors size entered is out of range. Must be between 1 and <?php echo $totalColors; ?> inclusive.</span>
            </p>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        for ($r = 0; $r < $rows; $r++) {
            ...
            foreach ($colorList as $color) {
                $selected = ($color['hex_value'] == $defaultColor['hex_value']) ? "selected" : "";
                echo "<option value='" . $color['hex_value'] . "' $selected>" . htmlspecialchars($color['name']) . "</option>";
            }
            echo "</select>";
            echo "</td>";
        
            echo "<td style='width:80%; background-color:" . $defaultColor['hex_value'] . ";' class='colorPreview'>";
            echo "</td>";
    ...
    <script>let COLORS_SIZE = <?php echo $totalColors; ?>;</script>
    ...
    document.querySelector("button[type='submit']").addEventListener("click", function() {
        ...
        document.querySelectorAll(".colorDropdown").forEach((dropdown, i) => {
            ...
            passColor.value = dropdown.options[dropdown.selectedIndex].text;
